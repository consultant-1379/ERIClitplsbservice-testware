...
public class LITPlsbserviceTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPlsbserviceTestRunner.class);
    ...
    @TestId(id = "CXP9031656-1", title = "Upgrade ERIClitplsbservice rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitplsbserviceRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("lsbservice");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031656-2", title = "Run python test cases for ERIClitplsbservice package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitplsbserviceTests() {
     ...
     * @PRE Execution of test {@link #runERIClitplsbserviceTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031656-3", title = "Parse xml outputs from python tests")
